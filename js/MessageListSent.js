(function() {
  ...
  class MessageListSent extends MessageList {
    ...
    deleteMessage(message) {
      // A delete republishes my data.json, so it must not run against an
      // unsynced/blank mailbox and wipe the real one.
      Page.user.guardContentWrite(function(may_write) {
        if (!may_write) return;
        var row = message.row;
